...
| Native content primitives (`Button`/`TextField`/`GroupedSection`) | 📱     | `native-ui-and-design-system.md`                           | two real bugs already found+fixed via source-level root-cause; on-screen still unverified    |
| Liquid Glass material (`GlassSurface`)                            | 📱     | `native-ui-and-design-system.md`                           | typechecks/bundles; on-screen unverified                                                     |
