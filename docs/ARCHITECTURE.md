...
- **Content primitives**: `Button`, `TextField`, `GroupedSection` are `@expo/ui`-backed (real SwiftUI/Jetpack Compose views) on the paths where that held up under an actual device build; see `README.md`'s "Known limitations" for the two bugs a device build surfaced and how they were fixed, and for the specific decision to keep `Icon`/`ChipSelect` as custom controls rather than convert them (documented regression-risk reasoning, not an oversight).
- **Glass material**: `GlassSurface` renders SwiftUI's real Liquid Glass (`@expo/ui`'s `glassEffect` modifier) on iOS, falling back to a flat opaque token surface on Android and when Reduce Transparency is on — Android has no equivalent native glass material, and forcing iOS's onto it would itself violate the native-first rule in the other direction.
